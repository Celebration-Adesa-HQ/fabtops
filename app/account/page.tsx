  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <Loader2
          className="w-12 h-12 text-pink-600 animate-spin"
          strokeWidth={1}
        />
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto">
        <ScrollReveal>
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            <div>
              <span className="text-[10px] uppercase tracking-[0.5em] font-black text-brand-dark mb-4 block opacity-40">
                Babe Circle Member
              </span>
              <h1 className="text-5xl md:text-7xl font-heading text-brand-dark uppercase leading-none">
                Hello,{" "}
                <span className="italic opacity-80">
                  {customer.firstName || "Babe"}
                </span>
              </h1>
            </div>
            <button
            ...
            <AnimatePresence mode="wait">
              {activeTab === "orders" && (
                <motion.div
                  ...
                  className="space-y-8"
                >
                  <h3 className="text-xs uppercase tracking-[0.3em] font-black text-brand-dark mb-8 border-b border-brand-dark/10 pb-4">
                    Recent Selections
                  </h3>
                  {orders.length > 0 ? (
                    <div className="space-y-6">
                      {orders.map(({ node: order }: any) => {
                        ...
                        return (
                          <div
                            key={order.id}
                            className="group bg-white/20 backdrop-blur-md border border-brand-dark/5 p-8 flex flex-col gap-8 hover:border-brand-dark transition-all rounded-[2rem]"
                          >
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 pb-8 border-b border-brand-dark/5">
                              <div className="flex items-center gap-8">
                                ...
                                <div>
                                  ...
                                  <h4 className="text-sm font-black uppercase text-brand-dark">
                                    {order.orderNumber
                                      ? `#${order.orderNumber}`
                                      : order.id.slice(-8)}
                                  </h4>
                              ...
                              <div className="flex items-center gap-12 text-center md:text-left">
                                <div>
                                  <p className="text-[9px] uppercase tracking-widest font-black text-brand-dark/30 mb-1">
                                    Status
                                  </p>
                                  <span
                                    className={`text-[10px] uppercase tracking-widest font-black bg-brand-dark p-2 rounded-lg ${order.fulfillmentStatus === "FULFILLED" ? "text-green-600" : "text-brand-primary animate-pulse"}`}
                                  >
                            ...
                            <div className="flex flex-wrap gap-4">
                              {items.map(({ node: item }: any, idx: number) => (
                                <div
                                  key={idx}
                                  className="flex items-center gap-4 bg-white/40 px-4 py-2 rounded-full border border-brand-dark/5"
                                >
                    ...
                    <div className="py-24 text-center border-2 border-dashed border-brand-dark/10 rounded-[3rem] bg-white/10 backdrop-blur-sm">
                      <Package
                        size={48}
                        className="mx-auto text-brand-dark/10 mb-6"
                        strokeWidth={1}
                      />
                      <p className="text-brand-dark/40 font-bold uppercase tracking-widest text-[11px] italic">
                        Your heritage journey begins with your first selection.
                      </p>
                      <Link
                        href="/shop"
                        className="mt-8 inline-block text-[10px] uppercase tracking-[0.4em] font-black text-brand-dark hover:text-brand-primary transition-colors underline underline-offset-8 decoration-brand-dark/10"
                      >
                        Browse Collection
                      </Link>
                    </div>
              ...
              {activeTab === "profile" && (
                <motion.div
                  ...
                  className="max-w-2xl"
                >
                  <h3 className="text-xs uppercase tracking-[0.3em] font-black text-brand-dark mb-12 border-b border-brand-dark/10 pb-4">
                    Personal Details
                  </h3>
                  <div className="grid md:grid-cols-2 gap-12">
                    <div className="space-y-2">
                      <p className="text-[9px] uppercase tracking-widest font-black text-brand-dark/30">
                        Full Identity
                      </p>
                      <p className="text-sm font-black text-brand-dark border-b border-brand-dark/5 pb-2 uppercase tracking-widest">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm font-black text-brand-dark border-b border-brand-dark/5 pb-2 uppercase tracking-widest">
                        {customer.defaultAddress?.city || "Unset"},{" "}
                        {customer.defaultAddress?.country || "Global"}
                      </p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-[9px] uppercase tracking-widest font-black text-brand-dark/30">
              ...
              {activeTab === "wishlist" && (
                ...
                >
                  <div className="flex justify-between items-end border-b border-brand-dark/10 pb-4 mb-12">
                    <h3 className="text-xs uppercase tracking-[0.3em] font-black text-brand-dark">
                      Styling Vault
                    </h3>
                    <span className="text-[9px] uppercase tracking-widest font-black text-brand-dark/40">
                      {favorites.length} Items Archived
                    </span>
                  </div>
                  ...
                  {favorites.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      {favorites.map((item) => (
                        <div
                          key={item.id}
                          className="group relative bg-white/20 backdrop-blur-md border border-brand-dark/5 p-6 rounded-[2rem] flex gap-6 hover:border-brand-dark transition-all"
                        >
                          ...
                          <div className="flex flex-col justify-between py-2">
                            <div>
                              <h4 className="text-[11px] font-black uppercase tracking-widest text-brand-dark mb-2 leading-tight">
                                {item.title}
                              </h4>
                    ...
                    <div className="py-24 text-center border-2 border-dashed border-brand-dark/10 rounded-[3rem] bg-white/10 backdrop-blur-sm">
                      ...
                      />
                      <p className="text-brand-dark/40 font-bold uppercase tracking-widest text-[11px] italic">
                        Your styling vault is currently empty.
                      </p>
                      <Link
                        href="/shop"
                        className="mt-8 inline-block text-[10px] uppercase tracking-[0.4em] font-black text-brand-dark hover:text-brand-primary transition-colors underline underline-offset-8 decoration-brand-dark/10"
