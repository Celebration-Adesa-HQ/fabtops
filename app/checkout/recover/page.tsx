  ...
  return (
    <main className="min-h-screen bg-brand-light px-6 pb-24 pt-32 md:px-12">
      <div className="mx-auto max-w-5xl">
        <Link href="/checkout" className="mb-12 inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-brand-dark/80 hover:text-brand-dark">
          Return to checkout
        </Link>
        ...
        <div className="grid gap-10 lg:grid-cols-[0.95fr,1.05fr]">
          <section className="rounded-[2rem] border border-brand-dark/8 bg-white/85 p-6 shadow-sm md:p-8">
            ...
            <p className="mt-6 text-sm leading-7 text-brand-dark/85">
            ...
            <div className="mt-10 space-y-6">
              ...
              <button
                ...
                disabled={loading}
                className="w-full rounded-full bg-brand-dark px-6 py-5 text-[10px] font-black uppercase tracking-[0.35em] text-brand-light transition hover:bg-brand-primary hover:text-brand-dark disabled:opacity-60"
              >
          ...
          <section className="rounded-[2rem] border border-brand-dark/8 bg-white/65 p-6 shadow-sm md:p-8">
            <p className="text-[10px] font-black uppercase tracking-[0.35em] text-brand-dark/75">Recovered Order</p>

            {!order ? (
              <div className="mt-8 rounded-[1.5rem] border border-dashed border-brand-dark/10 bg-brand-light/60 p-6 text-sm leading-7 text-brand-dark/85">
              ...
              <div className="mt-8 space-y-6">
                <div className="rounded-[1.5rem] border border-brand-dark/8 bg-white p-5">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-dark/75">Order #{order.id}</p>
                ...
                <div className="rounded-[1.5rem] border border-brand-dark/8 bg-white p-5">
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-dark/75">Line Items</p>
                    ...
                    {(order.items || []).map((item) => (
                      <div key={`${item.id}-${item.quantity}`} className="flex items-center justify-between gap-4 rounded-[1.25rem] border border-brand-dark/8 px-4 py-4">
                        <div>
                          ...
                          <p className="mt-1 text-[10px] uppercase tracking-[0.24em] text-brand-dark/75">Qty {item.quantity}</p>
                        </div>
                ...
                {needsRecovery(order.status) ? (
                  <div className="rounded-[1.5rem] border border-brand-primary/20 bg-brand-primary/10 p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-dark/75">Retry Payment</p>
                    ...
                    <label className="mt-5 block">
                      <span className="mb-2 block text-[10px] font-black uppercase tracking-[0.3em] text-brand-dark/75">Payment Method</span>
                    ...
                    <button
                      ...
                      disabled={retrying}
                      className="mt-6 w-full rounded-full bg-brand-dark px-6 py-4 text-[10px] font-black uppercase tracking-[0.35em] text-brand-light transition hover:bg-brand-primary hover:text-brand-dark disabled:opacity-60"
                    >
                      {retrying ? 'Restarting Payment...' : 'Retry Payment'}
                    </button>
                  </div>
                ) : (
                  <div className="rounded-[1.5rem] border border-brand-dark/8 bg-brand-light/60 p-5 text-sm leading-7 text-brand-dark/85">
