...
export function FloatingContact() {
  ...
  const contactOptions = [
    { icon: MessageCircle, label: 'WhatsApp', href: 'https://wa.me/234000000000', color: 'bg-[#25D366]' },
    { icon: Mail, label: 'Email Concierge', href: 'mailto:concierge@fabtops.com', color: 'bg-brand-dark' },
    { icon: InstagramIcon, label: 'Direct Message', href: 'https://instagram.com/fabtops', color: 'bg-brand-primary' }
  ];
  ...
  return (
    <div className="fixed bottom-10 right-10 z-[100] flex flex-col items-end gap-6">
        ...
        {isOpen && (
          <div className="flex flex-col items-end gap-4 mb-2">
            {contactOptions.map((option, i) => (
              ...
              >
                <span className="bg-white/80 backdrop-blur-md px-4 py-2 rounded-full text-[10px] uppercase tracking-widest font-black text-brand-dark shadow-xl opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
                  {option.label}
                </span>
                <div className={`${option.color} text-white w-14 h-14 rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition-transform`}>
      ...
      >
        ...
        {!isOpen && (
          <span className="absolute -top-2 -right-2 bg-brand-primary w-5 h-5 rounded-full border-4 border-brand-secondary" />
        )}
      </button>
    </div>
  );
